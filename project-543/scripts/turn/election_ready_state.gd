...
func validate() -> Array[String]:
	...
	if final_phase != "COMPLETED":
		errors.append("final_phase must be COMPLETED")
...
static func from_dictionary(data: Dictionary) -> S7ElectionReadyState:
	var result := S7ElectionReadyState.new()
	result.schema_version = int(data.get("schema_version", SCHEMA_VERSION))
	result.campaign_id = String(data.get("campaign_id", ""))
	result.final_week = int(data.get("final_week", 45))
	result.final_phase = String(data.get("final_phase", "COMPLETED"))
	result.final_political_state = data.get("final_political_state", {}).duplicate(true)
	result.final_campaign_state = data.get("final_campaign_state", {}).duplicate(true)
	result.final_economic_state = data.get("final_economic_state", {}).duplicate(true)
	result.final_risk_state = data.get("final_risk_state", {}).duplicate(true)
	result.final_constituency_states = data.get("final_constituency_states", {}).duplicate(true)
	result.action_history = data.get("action_history", []).duplicate(true)
	result.resolution_history = data.get("resolution_history", []).duplicate(true)
	result.model_versions = data.get("model_versions", {}).duplicate(true)
	return result
