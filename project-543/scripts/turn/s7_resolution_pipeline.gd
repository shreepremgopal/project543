...
func execute(
	...
	record.resolution_id = _make_resolution_id(
		str(state_before.get("campaign_id", "campaign_001")),
		week,
	...
	if money < total_cost:
		return {
			...
			"errors": [
				"required=%d available=%d" % [total_cost, money]
			]
		}
	...
	for action in actions:
		if action.cost > 0:
			...
			record.events_placeholder = null if false else null
	...
	for action in actions:
		var action_dict: Dictionary = action.to_dictionary()
		record.actions.append(action_dict)
	...
	for action in actions:
		var political_delta = int(
			action.parameters.get("political_delta", 0)
		)

		if political_delta != 0:
			var political_state: Dictionary = working_state.get(
				"political",
				{}
			).duplicate(true)

			var current_support: int = int(
				political_state.get("support", 0)
			)

			political_state["support"] = (
				current_support + political_delta
			)

			working_state["political"] = political_state

	record.stages_executed.append(
		POLITICAL_EFFECT_APPLICATION
	)
	...
	var risk_delta: int = 0

	for action in actions:
		risk_delta += int(
			action.parameters.get("risk_delta", 0)
		)

	if risk_delta != 0:
		var risk_state: Dictionary = working_state.get(
			"risk",
			{}
		).duplicate(true)

		var current_risk: int = int(
			risk_state.get("total", 0)
		)

		risk_state["total"] = current_risk + risk_delta
		working_state["risk"] = risk_state

		record.risk_changes.append({
			"week": week,
			"delta": risk_delta,
			"total": current_risk + risk_delta
		})
	...
	var saturation_delta: int = 0

	for action in actions:
		saturation_delta += int(
			action.parameters.get("saturation_delta", 0)
		)

	if saturation_delta != 0:
		var saturation_state: Dictionary = working_state.get(
			"saturation",
			{}
		).duplicate(true)

		var current_saturation: int = int(
			saturation_state.get("total", 0)
		)

		saturation_state["total"] = (
			current_saturation + saturation_delta
		)

		working_state["saturation"] = saturation_state

		record.saturation_changes.append({
			"week": week,
			"delta": saturation_delta,
			"total": current_saturation + saturation_delta
		})
	...
	for action in actions:
		income_amount += int(
			action.parameters.get("income", 0)
		)

	if income_amount != 0:
		working_state["money"] = (
			int(working_state.get("money", 0))
			+ income_amount
		)

		record.income.append({
			"week": week,
	...
	for effect in working_state.get("effects", []):
		var expiry_week: int = int(
			effect.get("expiry_week", -1)
		)
	...
	for action in actions:
		...
		if int(action.turn) != week:
			return {
				...
				"code": "INVALID_WEEK",
				"errors": [
					"action %s belongs to week %d, expected %d"
					% [action.action_id, action.turn, week]
				]
			}
...
func _build_effect(action, week: int):
	...
	var duration: int = int(
		action.parameters.get("duration", 0)
	)

	if duration > 0:
		effect["expiry_week"] = week + duration
	else:
		effect["expiry_week"] = -1

	return effect


func _make_resolution_id(
	campaign_id: String,
	week: int,
